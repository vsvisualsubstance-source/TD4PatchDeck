...
def onOffToOn(channel, sampleIndex, val, prev):
    ...
    old_deck_a = str(midi_table[1, 0].val or "0")
    old_deck_b = str(midi_table[1, 1].val or "0")
    ...
    # Spegni il/i patch PRECEDENTEMENTE sui deck ora sostituiti, se non sono
    # protetti altrove (sull'altro deck o ancora tra gli extra_active) --
    # altrimenti ogni cambio patch su un deck lascia il vecchio acceso per
    # sempre: e' esattamente la causa per cui piu' di 6 patch finivano
    # attive scorrendo/cambiando deck ripetutamente sull'APC40.
    new_deck_a = str(patch_num) if is_deck_a else old_deck_a
    new_deck_b = str(patch_num) if is_deck_b else old_deck_b
    still_authorized = {new_deck_a, new_deck_b} | {str(n) for n in led_ctrl._getExtraActive()}
    still_authorized.discard("0")

    stale_candidates = []
    if is_deck_a:
        stale_candidates.append(old_deck_a)
    if is_deck_b:
        stale_candidates.append(old_deck_b)

    for stale in stale_candidates:
        if stale != "0" and stale not in still_authorized:
            stale_comp = op(f'/PATCHDECK/PATCHES/X{stale}')
            if stale_comp:
                stale_comp.allowCooking = False
                print(f"[DECK] ⏏️ Patch {stale} spenta (sostituita sul deck)")

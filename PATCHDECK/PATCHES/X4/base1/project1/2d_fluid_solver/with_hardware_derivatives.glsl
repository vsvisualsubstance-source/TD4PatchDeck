...
vec2 curl_slope(vec2 uv)
{
    ...
    //vec2 slope = vec2(dFdx(curl(uv)) ,  dFdy(curl(uv))); - doesn't work
    //return abs(slope.yx) * sign(curl(uv));
   return vec2(abs(curl_r)-abs(curl_l),abs(curl_t)-abs(curl_b));// abs becuase sign gets added at vorticity stage
}
...
float divergence(vec2 uv)
{
  ...
  //can't use fwidth() because we need the sign
    float div = (dFdxFine(texture(sTD2DInputs[0],uv).x)+dFdyFine(texture(sTD2DInputs[0],uv).y)) * 0.5;
...
vec2 pressure_grad(vec2 uv)
{
    ...
    */
    //unfortunately looks ugly :(
        vec2 grad_hw = vec2(
            dFdxFine(pressure(uv)),
            dFdyFine(pressure(uv))
        );
     //   */
        return grad_hw;
    }
    ...
    void main()
    {
        ...
        velocity = any(isinf(velocity))||any(isnan(velocity))?vec2(0):velocity; //NaN prevention
        clear_press = (isinf(clear_press))||(isnan(clear_press))?0.0:clear_press; //NaN prevention

...
vec3 gradient(vec3 p) {
    const float eps = u_epsilon; 
    
    float center = exp_fbm(p);
    float dx = exp_fbm(p + vec3(eps, 0.0, 0.0)) - center;
    float dy = exp_fbm(p + vec3(0.0, eps, 0.0)) - center;
    float dz = exp_fbm(p + vec3(0.0, 0.0, eps)) - center;
